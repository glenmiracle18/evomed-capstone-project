"""
Prepare training data from BRCA Exchange and ClinVar datasets using live API
...
def process_brca_exchange_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Clinical_Significance_ENIGMA" in df.columns:
        processed["label"] = df["Clinical_Significance_ENIGMA"].apply(
            parse_clinical_significance
        )
    elif "Pathogenicity_expert" in df.columns:
        processed["label"] = df["Pathogenicity_expert"].apply(
            parse_clinical_significance
        )
    else:
    ...
    after = len(processed)
    print(
        f"   Filtered: {before:,} → {after:,} variants ({before - after:,} uncertain removed)"
    )
    ...
    if len(processed) > 0:
        ...
        print(
            f"   Pathogenic: {pathogenic:,} ({pathogenic / len(processed) * 100:.1f}%)"
        )
...
def process_clinvar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = (
        processed["review_status"]
        .str.lower()
        .str.contains("|".join(quality_keywords), na=False)
    )
...
def merge_datasets(dfs: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    merged = merged.drop_duplicates(
        subset=["chromosome", "position", "alt"], keep="first"
    )
    after = len(merged)
    print(
        f"   Deduplicated: {before:,} → {after:,} variants ({before - after:,} duplicates removed)"
    )
