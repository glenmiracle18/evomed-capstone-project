...
export interface AnalysisResult {
  ...
  // African Population-Specific Scoring fields
  evo2_delta_score?: number;
  population_adjusted_score?: number;
  population_adjustment?: number;
  adjustment_reasoning?: string;
  african_frequency?: number | null;
...
export async function analyzeVariantWithAPI({
  ...
  // Use the Next.js API proxy route for better security
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      use_african_adjustment: useAfricanAdjustment,
    }),
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to analyze variant (${response.status}): ${errorMessage}`,
    );
  }
