...
// Known African founder mutations from literature
const AFRICAN_FOUNDER_MUTATIONS: PopulationVariant[] = [
  {
    ...
    cancerRisk: "60-80% lifetime breast cancer risk, 40-50% ovarian cancer risk",
  ...
  },
  {
    ...
    pathogenicity: "Pathogenic",
    clinicalSignificance: "Pathogenic",
    cancerRisk: "50-70% lifetime breast cancer risk, 15-20% ovarian cancer risk",
  ...
  },
  {
    ...
    pathogenicity: "Pathogenic",
    clinicalSignificance: "Pathogenic",
    cancerRisk: "Li-Fraumeni syndrome - multiple cancer types, >90% lifetime risk",
...
function calculateRiskScore(request: RecommendVariantsRequest): RiskAssessment {
  ...
  if (request.personalHistory?.hasCancer) {
    ...
    if (request.personalHistory.ageAtDiagnosis && request.personalHistory.ageAtDiagnosis < 50) {
      score += 0.15;
  ...
  const firstDegreeRelatives = request.familyHistory.filter(member =>
    ['mother', 'father', 'sister', 'brother', 'daughter', 'son'].includes(member.relationship.toLowerCase())
  );

  const firstDegreeCases = firstDegreeRelatives.filter(member => member.hasCancer);

  if (firstDegreeCases.length >= 2) {
    score += 0.25;
    factors.push(`${firstDegreeCases.length} first-degree relatives with cancer`);
  ...
  const earlyOnsetFirstDegree = firstDegreeCases.filter(member =>
    member.ageAtDiagnosis && member.ageAtDiagnosis < 50
  );
  ...
  const secondDegreeRelatives = request.familyHistory.filter(member =>
    ['grandmother', 'grandfather', 'aunt', 'uncle', 'niece', 'nephew'].includes(member.relationship.toLowerCase())
  );

  const secondDegreeCases = secondDegreeRelatives.filter(member => member.hasCancer);

  if (secondDegreeCases.length >= 2) {
    score += 0.15;
    factors.push(`${secondDegreeCases.length} second-degree relatives with cancer`);
  ...
  const brcaCancers = request.familyHistory.filter(member =>
    member.hasCancer &&
    member.cancerType &&
  ...
  // Male breast cancer
  const maleBreastCancer = request.familyHistory.filter(member =>
    member.hasCancer &&
    member.cancerType?.toLowerCase().includes('breast') &&
    ['father', 'brother', 'grandfather', 'uncle', 'son'].includes(member.relationship.toLowerCase())
  );
  ...
  if (score >= 0.6) {
    riskLevel = "Very High";
    explanation = "Strong indication for genetic testing. Multiple high-risk factors present. Immediate genetic counseling recommended.";
  } else if (score >= 0.4) {
    riskLevel = "High";
    explanation = "Significant risk factors identified. Genetic testing strongly recommended.";
  } else if (score >= 0.2) {
    riskLevel = "Moderate";
    explanation = "Some risk factors present. Genetic testing may be beneficial. Consult with healthcare provider.";
  } else {
    riskLevel = "Low";
    explanation = "Limited family history of cancer. General population screening guidelines apply.";
  }
